  ...
  def start_link({names, mem_size, mem_strategy, module}) do
    ...
    after
      unless is_nil(mem_size) do
        :ets.new(names.lru_ets, [:set, :public, :named_table, write_concurrency: true])
        :ets.new(names.lru_inverted_ets, [:ordered_set, :public, :named_table, write_concurrency: true])
        cleaner =
